...
Hugging Face Spacesにアクセス後、以下の簡単な手順でサンフレッチェ広島の試合結果を予測できます。

1. **チームスタッツ（期待値など）の入力**
   - 画面の入力フォームに、予測したい試合の「サンフレッチェ広島のゴール期待値」や「対戦相手のゴール期待値」などの数値を入力します。
   - ※過去のスタッツを参考にしたい場合は、同階層にある `sanfrecce_real_data.csv` のデータも目安になります。

2. **予測シミュレーションの実行**
   - 入力を終えたら、画面下部の **「試合結果を予測する（Predict）」** ボタンをクリックします。

3. **予測結果・確率分布の確認**
   - ボタンを押すと、AIモデルが裏側で数千パターンの試合展開をシミュレーションします。
   - 画面下部に **「最も起こり得そうなスコアの組み合わせ（例: 2 - 1）」** や、**「勝ち・引き分け・負けの確率（%）」** が直感的なグラフィックで表示されます。
